...
//        }
        ...
        payNumber++;
        init_ReqChannel(ctx, price, str, product, extData, Did, receivers, false, receiver);
    }
    ...
    private void init_ReqChannel(final Context ctx, final String customized_price, final String str, final String product,
        ...
        init_ReqChannel(ctx, customized_price, product, extData, Did, receivers, receiver);
    }
                        ...
                        if (Through.id.equals(LASTREQUESTHROUGHID)) {
                            ...
                            if (System.currentTimeMillis() - LASTREQUESTTIME < Through.timing * 1000) {
                                if (throughCounter < THROUGNUMBER - 1) {
                                    PayThrough++;
                                    throughCounter++;
                                    init_ReqChannel(ctx, customized_price, "", product, extData, Did, receivers, true,
                                            callback);
                                }
        ...
        payChannel.addPayChannelListener(new IPayChannelListener() {
            ...
            public void onPaySucceeded() {
                ...
                if (throughCounter < THROUGNUMBER - 1) {
                    PayThrough++;
                    throughCounter++;
                    init_ReqChannel(ctx, price, "", productName, extData, Did, cb, true, callback);
                    cb.getOrderInfo().is_supplement = 1;
...
//                }
    ...
    private void goToNextThrough(Context ctx, String customized_price, String product, String extData, String Did, YPayCallbackInfo receivers, final Object callback) {
        if (throughCounter < THROUGNUMBER - 1) {
            PayThrough++;
            throughCounter++;
            init_ReqChannel(ctx, customized_price, "", product, extData, Did, receivers, true, callback);
